...
/**
 * Classe Responsável por interceptar todas as requisições para utilizar os
 * cookies para armazenar o refresh_token
 * 
 * @author luiz
 *
 */
...
public class RefreshTokenPostProcessor implements ResponseBodyAdvice<OAuth2AccessToken> {
	...
	// filtro para as requisições
	@Override
	public boolean supports(MethodParameter returnType, Class<? extends HttpMessageConverter<?>> converterType) {
		return returnType.getMethod().getName().equals("postAccessToken");
	}

	// metodo executado quando alguma requisição é interceptada
	@Override
	...
	private void adicionarRefreshTokenNoCookie(String refreshToken, HttpServletRequest req, HttpServletResponse res) {
		...
		refreshTokenCookie.setSecure(gerenciadorDeDespesasProperty.getSeguranca().isEnableHttps());
